...
window.renderCards = function () {
    ...
    if (data.length === 0) {
        container.append('<p>Nenhuma propriedade encontrado.</p>');
        return;
    }
        ...
        container.append(`
            <div class="col-10 card radius-10 border-start border-0 border-5 border-success borda m-3 ms-0"
                ...
                <div class="card-body row">
                    ...
                    <h4 class="card-text col-6 m-2">Cidade: ${item.Cidade}</h4>
                            <h4 class="card-text col-4 m-3">Estado: ${item.Estado}</h4>
                    <div class="row">
                        <a href="/Propriedade/Details/${item.Id}" class="borda btn borda-concluir btn-outline-success col-3 m-2">Detalhes</a>
                        <a href="/Propriedade/Delete/${item.Id}" class="borda btn border-danger btn-danger col-3 m-2">Excluir</a>
                    </div>
                </div>
            </div>
        `);
